...
from cafe_service.database.models.base import Base
from sqlalchemy import Boolean, CheckConstraint, String, Enum, Text
from sqlalchemy.orm import Mapped, mapped_column, relationship
from sqlalchemy import ForeignKey, UniqueConstraint, DateTime, func, Numeric, Integer
...
class ReservationStatusEnum(StrEnum):
    PENDING = "pending"
    CONFIRMED = "confirmed"
    ATTENDED = "attended"
    CANCELLED = "cancelled"
...
class ReservationModel(Base):
    ...
    cafe: Mapped["CafeModel"] = relationship("CafeModel")
    # Зв'язок один-до-одного з замовленням
    order: Mapped["OrderModel"] = relationship("OrderModel", back_populates="reservation", uselist=False)

    __table_args__ = (
        CheckConstraint("number_of_guests > 0", name="ck_reservation_guests_positive"),
    )
...
class OrderModel(Base):
    ...
    # Зв'язок з щойно створеною моделлю
    reservation_id: Mapped[int | None] = mapped_column(
        ForeignKey("reservations.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
